...
A central data structure in text mining is the Document Term Matrix (DTM). The DTM serves as the input into many machine learning models. 

There are multiple tools for producing a DTM. Once you have the DTM in hand, you can use it with many ML models.

### Tidyverse
Tidyverse tools for text mining encompass data input, cleansing, and tokenization. Tidy structures cast to a DTM.

*Reference*: [Text Mining with R: A Tidy Approach](https://www.tidytextmining.com).

### Machine learning
We classified emails by employing a Support Vector Machine model from the `caret` package.

### Other text mining tools
We also tried other text mining tools in order to become familiar with prevailing methodologies. We took the tools out for test drives and explored other uses.

- We built a corpus with the `tm` package, and did yadda yadda yadda.
- We used the `quanteda` package to ...
- Whatever else we did... The sentiment analysis, for example, et cetera.

## Read emails
The processing begins here using a Tidy approach.

Read two directories of emails. `readr::read_file()` is very fast and syntactically easier than `base::readLines()`. We processed 500 emails from each class as a pragmatic limitation of computation resources.
...
## Clean
Remove headers. By specification, email headers may not contain blank lines. The regular expression strips them.

**Regular expression notes**:

- RegEx: `.*?\n\n`
- The question mark enables non-greedy matching of any text in order to match up to the first double line break.
- The solution isn't perfect. There are some headers with multiple instances of blank lines. Nevertheless, most of the headers are removed and the solution appears adequate.
...
## Segment data
Segment the data into sets for training and testing.
